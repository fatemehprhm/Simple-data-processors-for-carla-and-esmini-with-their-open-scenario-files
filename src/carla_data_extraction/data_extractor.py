...
def main():
    ...
    while flag:
        if world.tick():
            ...
            data = {
                'time': t_time,
                'ego_speed': round(ego_speed, 10)*10/36, 
                'ego_acc_x': round(imu_sensor.accelerometer[0], 10),
                'ego_acc_y': round(imu_sensor.accelerometer[1], 10),
                'ego_acc_z': round(imu_sensor.accelerometer[2], 10),
                'ego_location': round(math.sqrt(t.location.x**2 + t.location.y**2), 10),
                'lead_speed': round(leading_speed, 10)*10/36,
                'lead_acc_x': round(imu_sensor2.accelerometer[0], 10),
                'lead_acc_y': round(imu_sensor2.accelerometer[1], 10),
                'lead_acc_z': round(imu_sensor2.accelerometer[2], 10),
                'lead_location': round(math.sqrt(t1.location.x**2 + t1.location.y**2), 10)}
